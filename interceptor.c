...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// Check if pid is monitored (if not, then we do not log the message and just
  // proceed to call the original system call)
	if (table[syscall].monitored != 2) {
		if (table[syscall].monitored == 0 || (table[syscall].monitored == 1 && !check_pid_monitored(syscall, pid))) {
...
/**
 * Takes syscall as an argument, and checks if it is in the range of valid
 * syscall numbers, and is not the same as MY_CUSTOM_SYSCALL (which we have
 * hijacked).
 *
 * Returns -1 if syscall is invalid, and 0 if it is valid.
 */
static int check_syscall_validity(int syscall) {

	if (syscall < 0 || syscall > NR_syscalls - 1 || syscall == MY_CUSTOM_SYSCALL) {
		printk(KERN_ERR "Syscall %d invalid\n", syscall);
		return -1;
...
/**
 * Takes a pid as an argument, and checks if it is non-negative and belongs
 * to a process.
 *
 * Returns -1 if pid is invalid, and 0 if it is valid.
 */
static int check_pid_validity(int pid) {

	if (pid < 0) {
		printk(KERN_ERR "Invalid pid: %d\n", pid);
		return -1;

	} else if (!pid_task(find_vpid(pid), PIDTYPE_PID) && pid != 0) {
		printk(KERN_ERR "pid does not belong to a valid process: %d\n", pid);
		return -1;
...
/**
 * Takes an argument uid and checks if the current_uid is equal to that.
 *
 * Returns 0 if equal to uid and -1 otherwise.
 */
static int check_caller_uid(int uid) {

	if (current_uid() != uid) {
		printk(KERN_ERR "uid does not have sufficient priviledges: %d", uid);
		return -1;
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if (check_syscall_validity(syscall) < 0)
		return -EINVAL;
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		...
		// Check permissions (caller must be root, which has a uid of 0)
		if (check_caller_uid(0) < 0)
			return -EPERM;

		// Check if system call already intercepted
		if (table[syscall].intercepted)
			return -EBUSY;

		// Modify table metadata to intercept syscall
		spin_lock(&my_table_lock);
		table[syscall].f = sys_call_table[syscall];
		table[syscall].intercepted = 1;
		spin_unlock(&my_table_lock);

		// Change pointer in sys_call_table to point to interceptor
	...
	// ***************************** //
		...
		// Check not releasing a system call that has not been intercepted
	...
	// ****************************** //
		...
		// Check pid validity
		if (check_pid_validity(pid) < 0)
			return -EINVAL;

		// Check permissions (if caller is not root but pid is 0,
		// and if pid request is owned by calling process)
		if (check_caller_uid(0) < 0) {
			if (!pid || check_pids_same_owner(current->pid, pid))
				return -EPERM;
		}
		...
		// Set monitored and add pid to table[syscall]
		// add_pid_sysc will return -ENOMEM if there is insufficient memory, and we
		// propogate that error through syscall
		spin_lock(&my_table_lock);
		if (!pid) {
	...
	// ***************************** //
		...
		// Check not stopping on system call that is not being monitored or has not been intercepted
		if (table[syscall].monitored != 2) {
			if (!check_pid_monitored(syscall, pid))
				return -EINVAL;
		}
		...
		// Stop monitoring pid for syscall
		spin_lock(&my_table_lock);
		table[syscall].listcount--;
...
static int init_function(void) {
	...
	// For looping through table and initializing list_heads
	int i;
	...
	// Initialize list heads for all entries in table. The other fields are
	// already initialized (and will be again when we intercept / de-intercept)
	// a systemm call) so we do not need to initialie them.
	spin_lock(&my_table_lock);
	...
	// Store original syscall and exit group, and put the addresses of ours in
	// the sys_call_table
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long) sys_call_table);
...
static void exit_function(void) {
	...
	// Restore sys_call_table to its original state
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long) sys_call_table);
